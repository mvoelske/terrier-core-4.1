...
public class HadoopPlugin implements ApplicationSetup.TerrierApplicationPlugin
{
	...
	/** Get a JobFactory with the specified session name. This method attempts three processes, in order:
	  * <ol>
	  * <li>If the current/default Hadoop configuration has a real Hadoop MapReduce/YARN cluster configured, then
	  * that will be used. This requires that the <tt>mapreduce.framework.name</tt> property in the hadoop configuration
	  * be configured.</li>
	  * <li>As a last resort, Terrier will use the local job tracker that Hadoop provides on the localhost. This is
	  * useful for unit testing, however it does not support multiple reducers, not task side-effect files.</li> 
	  * </ol>
	  */
	...
	protected static JobFactory getJobFactory(String sessionName, boolean persistent) {
		...
		try {
			String frameworkName = globalConf.get("mapreduce.framework.name");
			//see if the current hadoop configuration is using YARN
			if (frameworkName == null)
			{
				frameworkName = new JobConf().get("mapreduce.framework.name");
			}
			if (frameworkName != null && ! frameworkName.equals("local"))
			{
				if (logger.isDebugEnabled()) logger.debug("Default configuration has MR configuration using framework " + frameworkName);	
